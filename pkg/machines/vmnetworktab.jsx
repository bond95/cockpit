...
const VmNetworkTab = function ({ vm, dispatch, hostDevices }) {
    ...
    const sourceJump = (source) => {
        return () => {
            if (source !== null && hostDevices.indexOf(source) > -1) {
    ...
    const detailMap = [
        ...
        { name: _("Source"), value: (network, networkId) => {
            const mapSource = {
                direct: (source) => <span className="machines-network-source-link" onClick={sourceJump(source.dev)}>{source.dev}</span>,
                network: (source) => <span className="machines-network-source-link" onClick={sourceJump(source.network)}>{source.network}</span>,
                bridge: (source) => <span className="machines-network-source-link" onClick={sourceJump(source.bridge)}>{source.bridge}</span>,
        ...
        { name: _("Action"), value: (network) => {
            const isUp = network.state === 'up';
            return (
                <div className='machines-network-state-button' >
                    {isUp ? 'Unplug' : 'Plug'}
            ...
            <Listing columnTitles={detailMap.map(target => target.name)} actions={null}>
                {vm.interfaces.sort().map(target => {
                    const columns = detailMap.map(d => {
                        ...
                        // if (d.tight) {
                        //     if (typeof column === 'string' || typeof column === 'number' || column === null || column === undefined || column instanceof String || React.isValidElement(column)) {
                        //         column = { name: column, tight: d.tight };
                        //     } else {
                        //         column.tight = true;
                        //     }
                        // }
                    ...
                    // const sourceJump = () => {
                    //     if (getSource(target) !== null && hostDevices.indexOf(getSource(target)) > -1) {
                    //         cockpit.jump(`/network#/${getSource(target)}`, cockpit.transport.host);
                    //     }
                    // };
                    networkId++;
