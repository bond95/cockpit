...
class HostVmsList extends React.Component {
    constructor(props) {
        ...
        this.deviceProxyAddHandler = this.deviceProxyAddHandler.bind(this);
        this.deviceProxyRemoveHandler = this.deviceProxyRemoveHandler.bind(this);
        this.state = { hostDevices: []};
        this.client = cockpit.dbus("org.freedesktop.NetworkManager", {});
    }

    componentWillMount() {
        const deviceProxies = this.client.proxies("org.freedesktop.NetworkManager.Device");
        deviceProxies.addEventListener('added', this.deviceProxyAddHandler);
        deviceProxies.addEventListener('changed', this.deviceProxyHandler);
        deviceProxies.addEventListener('removed', this.deviceProxyRemoveHandler);
    }
    ...
    deviceProxyAddHandler(event, proxy) {
        const devices = this.state.hostDevices;
        if (devices.indexOf(proxy.Interface) === -1) {
            devices.push(proxy.Interface);
        }
        this.setState({ hostDevices: devices });
    }

    deviceProxyHandler(event, proxy) {
        const devices = this.state.hostDevices;
        const index = devices.indexOf(proxy.Interface);
        if (index > -1 && !proxy.valid) {
            devices.splice(index, 1);
        }
        this.setState({ hostDevices: devices });
    }

    deviceProxyRemoveHandler(event, proxy) {
        const devices = this.state.hostDevices;
        const index = devices.indexOf(proxy.Interface);
        if (index > -1) {
            devices.splice(index, 1);
                ...
                {vms
                    ...
                    return (
                        <Vm vm={vm} config={config}
                            hostDevices={that.state.hostDevices}
